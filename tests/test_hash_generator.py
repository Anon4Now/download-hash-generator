...
def test_hash_class() -> None:
    ...
    test_file_read = "test_file_read.txt"

    # Create/Write to a file to emulate the temp_file being created by the script
    with open(test_file_path, 'w+') as f:  # create "event" file
        f.write(test_file_read)  # write the path to another "downloaded file" in it
    ...
    hashing = hash_generator.Hash(text_file=test_file_path)
    ...
    if os.path.exists(test_file_read):
